...
class Orto(object):
    ...
    def get_button_list(self):
        ...
        # Wybór pytania
        self.problem = random.randint(0, len(self.data)-1)
        ...
        if choice == 1:
            ...
            btn = Button(posX, posY, width, height, (self.data[self.problem][3]),self.game)
            button_list.append(btn)
    ...
    def get_data(self):
        ...
        if self.level == 0:
            data = Read('dane/ort_easy.txt')
        elif self.level == 1:
            data = Read('dane/ort_medium.txt')
        elif self.level == 2:
            data = Read('dane/ort_hard.txt')
